...
- [ ] **继续 dashboard 可视化**(下步):① 项目进度区(In progress)从纯文字列表 → 更视觉的卡片/环形进度;② 可加一条总览趋势图(采购额/省钱随时间);③ 右栏「Replies Layla is handling」仍偏文字,可加状态可视化;④ 视情况加更多克制科技点缀(hairline / 微光 / glass)。谨慎、逐屏、勿 slop。
